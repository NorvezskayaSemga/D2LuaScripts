...
local _setValue_Crit1DamageSpellBuffMultiplier = {}
local _setValue_Crit2DamageSpellBuffMultiplier = {}
function svSetCrit1InitialDamage(unit, value, ratio, totalDmg, spellBuffMutiplier)
	local id = unit.id.value
	_svCurrentCrit1Value[id] = value
	_svCurrentCrit1Ratio[id] = ratio
	_svCurrentCrit1TotalDamage[id] = totalDmg
	_setValue_Crit1DamageSpellBuffMultiplier[id] = spellBuffMutiplier
end
function svSetCrit2InitialDamage(unit, value, ratio, totalDmg, spellBuffMutiplier)
	local id = unit.id.value
	_svCurrentCrit2Value[id] = value
	_svCurrentCrit2Ratio[id] = ratio
	_svCurrentCrit2TotalDamage[id] = totalDmg
	_setValue_Crit2DamageSpellBuffMultiplier[id] = spellBuffMutiplier
end
...
function svAddCrit1Damage_Spell(unit, prev, value)
	local id = unit.id.value
	return svAddCrit1Damage(unit, prev, value * _setValue_Crit1DamageSpellBuffMultiplier[id])
end

function svAddCrit2Damage_Spell(unit, prev, value)
	local id = unit.id.value
	return svAddCrit2Damage(unit, prev, value * _setValue_Crit2DamageSpellBuffMultiplier[id])
end
