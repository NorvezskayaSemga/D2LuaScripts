...
function _mapMultipliers_getValue(unit, suffix, modifierToIgnore, player)
	...
	local p
	if player == -1 then
		p = _GroupInfo_getLastFoundGroupPlayer()
	else
		p = player
	end
	if p ~= nil then
...
function _mapMultipliers_Damage1_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_DAMAGE1_MULTI', _mapMulti_ignore_DAMAGE1_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Damage2_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_DAMAGE2_MULTI', _mapMulti_ignore_DAMAGE2_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Heal1_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_HEAL1_MULTI', _mapMulti_ignore_HEAL1_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Heal2_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_HEAL2_MULTI', _mapMulti_ignore_HEAL2_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Power1_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_POWER1_MULTI', _mapMulti_ignore_POWER1_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Power2_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_POWER2_MULTI', _mapMulti_ignore_POWER2_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Initiative_Multi(unit)
	local v = _mapMultipliers_getValue(unit, '_INITIATIVE_MULTI', _mapMulti_ignore_INITIATIVE_MULTI, -1)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_Armor_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_ARMOR_FLAT', _mapMulti_ignore_ARMOR_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_Regen_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_REGEN_FLAT', _mapMulti_ignore_REGEN_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_Negotiate_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_NEGOTIATE_FLAT', _mapMulti_ignore_NEGOTIATE_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_Drain1_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_DRAIN1_FLAT', _mapMulti_ignore_DRAIN1_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_Drain2_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_DRAIN2_FLAT', _mapMulti_ignore_DRAIN2_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_XpGain_Multi(unit, player)
	local v = _mapMultipliers_getValue(unit, '_XPGAIN_MULTI', _mapMulti_ignore_XPGAIN_MULTI, player)
	if v == nil then
		return 1
	else
		return 0.01 * v
...
function _mapMultipliers_MovementPoints_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_MOVEMENT_FLAT', _mapMulti_ignore_MOVEMENT_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_Scout_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_SCOUT_FLAT', _mapMulti_ignore_SCOUT_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_TradeDiscount_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_TRADEDISCOUNT_FLAT', _mapMulti_ignore_TRADEDISCOUNT_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_CritDamage1_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_CRITDAMAGE1_FLAT', _mapMulti_ignore_CRITDAMAGE1_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_CritDamage2_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_CRITDAMAGE2_FLAT', _mapMulti_ignore_CRITDAMAGE2_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_CritPower1_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_CRITPOWER1_FLAT', _mapMulti_ignore_CRITPOWER1_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
...
function _mapMultipliers_CritPower2_Flat(unit)
	local v = _mapMultipliers_getValue(unit, '_CRITPOWER2_FLAT', _mapMulti_ignore_CRITPOWER2_FLAT, -1)
	if v == nil then
		return 0
	else
		return v - 100
