...
Imports D2LuaScripts



'''<summary>
'''This is a test class for DoppelgangerTest and is intended
'''to contain all DoppelgangerTest Unit Tests
'''</summary>
<TestClass()> _
    ...
    Public Sub LevelToExpTest()
        ...
        For p As Integer = 0 To UBound(expected) Step 1
            For currentLevel As Integer = 0 To UBound(expected(p)) Step 1
                actual1 = Doppelganger.LevelToExpFast(baseLevel, currentLevel, dynLevel, baseExp, expNext1(p), expNext2(p))
                actual2 = Doppelganger.LevelToExpSlow(baseLevel, currentLevel, dynLevel, baseExp, expNext1(p), expNext2(p))
        ...
        For p As Integer = 0 To UBound(expNext1) Step 1
            For currentLevel As Integer = baseLevel To 20 Step 1
                For add As Integer = 0 To 5 Step 1
                    ...
                    currentExp = Doppelganger.LevelToExpFast(baseLevel, currentLevel, dynLevel, baseExp, expNext1(p), expNext2(p))
                    currentExp += addExp
                    actual = Doppelganger.ExpToLevelSlow(currentExp, baseLevel, dynLevel, baseExp, expNext1(p), expNext2(p))
